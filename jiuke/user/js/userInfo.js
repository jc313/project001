	...
	requirejs(['avalon','bootstrap','ajax','util','deferred'], function(avalon,bootstrap,ajax,util,deferred) {
		avalon.ready(function () {
			...
			vm = avalon.define({
				...
				tab1:function(e,href,h){
					if(vm.hide1){
						$('#login').modal('show');
						return
					}
					avalon.each(document.getElementsByClassName('li'), function(index, el){
						avalon(el).removeClass('active1')
						avalon(el).element.innerHTML = avalon(el).element.innerText
					})
					e.currentTarget.className = 'li active1'
					e.currentTarget.innerHTML = e.currentTarget.innerText+'<span class="glyphicon glyphicon-chevron-right"></span>';
					vm.height = h;
					vm.iframe = href;
				},
			});
