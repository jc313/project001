	...
	requirejs(['avalon','bootstrap','ajax','util','deferred'], function(avalon,bootstrap,ajax,util,deferred) {
		...
		**/
		function newsDetail(){
			postToServer('eduWeb/news/newsDetail',{sid:vm.sid,userId:vm.userId},function(json){
				if(json.code == 2000){
					try{
						vm.pic = json.data.picList[0].realPicUrl;
					}catch(e){}
					vm.title = json.data.news.title;
					vm.createTime = json.data.news.createTime;
					vm.content = json.data.news.content;
				}else{
					alert(json.message)
				}
			})
